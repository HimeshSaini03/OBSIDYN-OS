...
class RhythmProfile:
    ...
    def _get_profile_path(self):
        ...
        return os.path.join(project_root, "config", "auth_profile.json")
    ...
    def _load_profile(self):
        ...
        try:
            ...
            loaded = FileUtils.read_file(self.profile_path)
            parsed = json.loads(loaded.decode("utf-8"))
    ...
    def _score_sample(self, summary):
        ...
        for metric in self.METRIC_KEYS:
            ...
            std = max(self._std(baseline), mean * 0.12, 8.0 if "duration" in metric else 2.5)
            score_total += abs(summary.get(metric, 0.0) - mean) / std

        expected_keys = self._mean([entry.get("key_count", 0) for entry in samples])
        key_penalty = abs(summary.get("key_count", 0) - expected_keys) / max(expected_keys, 1.0)
