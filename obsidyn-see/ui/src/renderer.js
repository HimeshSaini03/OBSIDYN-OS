...
//     console.log('[OBSIDYN] Setting up event listeners...');
...
//     } else {
//         console.warn('[STEG] ✗ btn-select-data-file not found');
//     }
...
//     } else {
//         console.warn('[STEG] ✗ btn-select-carrier not found');
//     }
...
//     let loading = document.getElementById('steg-loading');
//     if (!loading) {
//         loading = document.createElement('div');
//         loading.id = 'steg-loading';
//         loading.className = 'steg-loading hidden';
//         loading.innerHTML = `
...
//             setTimeout(() => animateNext(index + 1), 3);
//         }, 30);
//     };
...
// // Start - with DOM ready check
// if (document.readyState === 'loading') {
//     console.log('[OBSIDYN] Waiting for DOM...');
//     document.addEventListener('DOMContentLoaded', () => {
//         console.log('[OBSIDYN] DOM ready!');
//         init();
//     });
// } else {
//     console.log('[OBSIDYN] DOM already ready');
//     init();
// }
...
// ===== PROFESSIONAL SHRED FEATURE =====
function setupShredFeature() {
    console.log('[SHRED] Setting up professional shred feature...');
    
    const dropZone = document.getElementById('shred-drop-zone');
    const btnShredFile = document.getElementById('btn-shred-file');
    const btnRemoveFile = document.getElementById('btn-remove-file');
    const btnExecuteShred = document.getElementById('btn-execute-shred');
    const btnShredAnother = document.getElementById('btn-shred-another');
    const shredConfirmCheck = document.getElementById('shred-confirm-check');
    
    // File selection via button
    if (btnShredFile) {
        btnShredFile.addEventListener('click', async () => {
            console.log('[SHRED] Select file clicked');
            try {
                const files = await ipcRenderer.invoke('select-file');
                if (files && files[0]) {
    ...
    if (btnRemoveFile) {
        btnRemoveFile.addEventListener('click', () => {
            ...
            const fileInfo = document.getElementById('shred-file-info');
            const shredOptions = document.getElementById('shred-options');
            const executeBtn = document.getElementById('btn-execute-shred');
            
            if (fileInfo) fileInfo.classList.add('hidden');
            if (shredOptions) shredOptions.classList.add('hidden');
    ...
    // Execute shred
    if (btnExecuteShred) {
        btnExecuteShred.addEventListener('click', () => {
            if (selectedFileForShred) {
                const confirmCheck = document.getElementById('shred-confirm-check');
                if (confirmCheck && confirmCheck.checked) {
                    executeSecureShred(selectedFileForShred);
                } else {
                    showNotification('Please confirm you understand this cannot be undone', 'error');
                }
            }
        });
    }
    ...
    // Shred another file
    if (btnShredAnother) {
        btnShredAnother.addEventListener('click', () => {
            const successModal = document.getElementById('shred-success-modal');
            const fileInfo = document.getElementById('shred-file-info');
            const shredOptions = document.getElementById('shred-options');
            const executeBtn = document.getElementById('btn-execute-shred');
    ...
    console.log('[SHRED] ✓ Professional shred feature ready');
}

function selectFileForShred(filePath) {
    selectedFileForShred = filePath;
    
    const fileName = filePath.split('\\').pop() || filePath.split('/').pop() || filePath;
    
    const filenameEl = document.getElementById('shred-filename');
    const filepathEl = document.getElementById('shred-filepath');
    ...
    if (shredOptions) shredOptions.classList.remove('hidden');
    if (executeBtn) executeBtn.classList.remove('hidden');
    
    showNotification('File selected for shredding', 'success');
}

async function executeSecureShred(filePath) {
    console.log('[SHRED] Executing secure shred:', filePath);
    
    // Show progress modal
    const progressModal = document.getElementById('shred-progress-modal');
    if (progressModal) {
    ...
    sendCommand('SHRED_FILE', { path: filePath });
}
...
async function simulateShredProgress() {
    ...
    for (let stepIndex = 0; stepIndex < steps.length; stepIndex++) {
        ...
        // Update pass counter
        if (stepIndex === 0) {
            const passEl = document.getElementById('shred-pass');
            if (passEl) passEl.textContent = '3/3';
        }
    }
    
    // Update overall progress
    const overallBar = document.getElementById('shred-progress-bar');
    const percentEl = document.getElementById('shred-percent');
    const timeEl = document.getElementById('shred-time');
...
async function executeScan() {
    ...
    console.log('[STEG] Scanning image:', imageFile);
    showNotification('Analyzing image...', 'info');
...
// ===== SHRED (LEGACY - KEPT FOR BACKWARDS COMPAT) =====
async function handleShred() {
    // This is the old simple shred - kept for compatibility
    // The professional UI uses setupShredFeature() instead
    try {
        const result = await ipcRenderer.invoke('select-file');
        if (result && result.length > 0) {
            const fileName = result[0].split('\\').pop();
    ...
    try {
        ...
        switch (data.status) {
            ...
            case 'SUCCESS':
                ...
                // Handle shred success for professional UI
                if (data.data && data.data.includes('destroyed')) {
                    handleShredSuccess();
                }
                if (data.data && (data.data.includes('Locked') || data.data.includes('Unlocked') || data.data.includes('Deleted') || data.data.includes('hidden') || data.data.includes('extracted'))) {
                    setTimeout(() => loadVaultList(), 500);
            ...
            case 'OK':
                ...
                if (data.action === 'SCAN_IMAGE' && data.data) {
                    renderScanResult(data);
                }
                if (data.data && data.data.image_info) {
                    renderScanResult(data);
...
// ===== SCAN RESULT RENDERER =====
function renderScanResult(data) {
    console.log('[STEG] Rendering scan result:', data);
    
    const resultContainer = document.getElementById('scan-result');
    const content = document.getElementById('scan-result-content');
    
    if (!resultContainer || !content) {
        console.error('[STEG] Scan result elements not found!');
        return;
    }
    
    const scanData = data.data || {};
    const hasData = scanData.has_hidden_data;
    const isObsidyn = scanData.is_obsidyn;
    
    // Build result HTML
    let html = `
        <div class="scan-result-item">
            <span class="scan-result-label">Hidden Data</span>
            <span class="scan-result-value ${hasData ? 'success' : ''}">
                ${hasData ? '✓ Detected' : '✗ None'}
            </span>
        </div>
    `;
    ...
    if (isObsidyn) {
        html += `
            ...
            <div class="scan-result-item">
                <span class="scan-result-label">Version</span>
                <span class="scan-result-value">v${scanData.version || '1'}</span>
            </div>
        `;
    }
    ...
    html += `
        <div class="scan-result-item">
            <span class="scan-result-label">Status</span>
            <span class="scan-result-value ${isObsidyn ? 'success' : hasData ? 'warning' : ''}">
                ${scanData.message || 'Scan complete'}
            </span>
        </div>
    `;
    ...
    console.log('[STEG] Scan result displayed');
    showNotification(scanData.message || 'Scan complete', hasData ? 'success' : 'info');
}
